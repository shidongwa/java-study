...
/**
 * 基数排序可以说是扩展了的桶式排序，比如当待排序列在一个很大的范围内，比如0到999999内，
 * 那么用桶式排序是很浪费空间的。而基数排序把每个排序码拆成由d个排序码，
 ...
 * 基数排序借助桶式排序，每次按第N位排序时，采用桶式排序。对于如何安排每次落入同一个桶中的数据
 * 有两种安排方法：
...
public class RadixSorter {
    ...
    private void link_radix_sort(int[] keys, int from, int len, int radix, int d) {
    ...
    private void link_radix_sort_distribute(int[] keys, int from, int len,
